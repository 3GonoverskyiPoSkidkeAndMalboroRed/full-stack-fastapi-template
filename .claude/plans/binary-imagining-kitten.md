...
- Hero на главной — статичный промо-блок «Ре́ Spring 2026» со ссылкой в каталог.
...
1. **Hero** — `<section class="hero">` grid 1fr/1fr с `border-bottom: 1px var(--ink)`:
   - слева `hero-meta`: тег `<span class="tag solid">Магазин</span>` + mono-мета «Раздел / 01 · Коллекция»; `<h1>` «Ре́ <em>Spring 2026</em>» (em в инверсии с skew); `<p class="lede">` краткое описание; `<Link to="/catalog" class="read-link">` «Открыть каталог →»; снизу mono «10 SKU · ДРОП №04»;
